...
pub fn new<'a, V:VerticesClient, B:BufferClient> (eg:&'a mut ExampleVertices<'a, V, B>, size:f32)  {
    let vertex_data = [-size,-size,0.0, size,-size,0.0, 0.0,size, 0.0];
    let index_data = [0u8,1,2];

    let indices = eg.push_data(Box::new(index_data));
    let vertices = eg.push_data(Box::new(vertex_data));
